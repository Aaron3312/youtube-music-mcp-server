...
class APIConfig(BaseModel):
    ...
    rate_limit_per_minute: int = Field(default=30, description="API requests per minute")
    ...
    user_agent: str = Field(default="YouTube Music MCP Server/2.0.0", description="User agent for API requests")
...
class ServerConfig(BaseModel):
    ...
    # OAuth configuration
    oauth_client_id: str = Field(..., description="Google OAuth client ID")
    oauth_client_secret: str = Field(..., description="Google OAuth client secret")

    # Optional configurations
    redis_url: Optional[str] = Field(default=None, description="Redis connection URL")
    encryption_key: str = Field(..., description="Encryption key for secure storage")
    ...
    def security_config(self) -> SecurityConfig:
        ...
        return SecurityConfig(encryption_key=self.encryption_key)
